...
class SCJNAnalyzer:
    ...
    def procesar_documento_con_timeout(self, ruta_archivo: Path, expediente: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Guardar JSON individual
            nombre_salida = f"{ruta_archivo.stem}_mapeado.json"
            ruta_salida = self.dirs['mapeos'] / expediente / nombre_salida
            ...
            entrada_bitacora = BitacoraEntry(
                ...
                documento=ruta_archivo.name,
                status="success",
                mensaje=f"Documento procesado exitosamente. Tokens: {tokens_usados}",
                metadata=metadata
            )
            self.bitacora.append(entrada_bitacora)
            ...
            print(f"  ✅ Completado. Tokens: {tokens_usados}, Tiempo: {metadata.tiempo_procesamiento:.2f}s")
            
            return resultado_json
            
        except Exception as e:
            # Registrar error en bitácora
            error_entry = BitacoraEntry(
                timestamp=datetime.now(),
                expediente=expediente,
                documento=ruta_archivo.name,
                status="error",
            ...
            self.bitacora.append(error_entry)
...
if __name__ == "__main__":
    ...
    def guardar_bitacora(self, expediente: str):
        """Guarda la bitácora del procesamiento"""
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        nombre_bitacora = f"bitacora_{expediente}_{timestamp}.json"
        ruta_bitacora = self.dirs['bitacoras'] / nombre_bitacora
        
        # Convertir bitácora a diccionarios serializables
        bitacora_dict = [entry.dict() for entry in self.bitacora]
        
        # Agregar resumen del expediente
        info_expediente = ExpedienteInfo(
            numero_expediente=expediente,
            fecha_inicio=datetime.now(),
            documentos_procesados=[entry.documento for entry in self.bitacora if entry.status == "success"],
            total_documentos=len([entry for entry in self.bitacora if entry.status == "success"]),
            tokens_totales=self.tokens_totales,
            tiempo_total_procesamiento=self.tiempo_total
        )
        
        bitacora_completa = {
            "resumen_expediente": info_expediente.dict(),
            "bitacora_detallada": bitacora_dict
        }
        
        with open(ruta_bitacora, 'w', encoding='utf-8') as f:
            json.dump(bitacora_completa, f, indent=2, ensure_ascii=False, default=str)
        
        print(f"\n📋 Bitácora guardada: {ruta_bitacora}")
